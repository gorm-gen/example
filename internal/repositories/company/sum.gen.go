...
// Sum SUM数据
func (c *Company) Sum(genField field.Expr) *_sum {
	return &_sum{
		core:          c,
		unscoped:      c.unscoped,
		genField:      genField,
...
func (s *_sum) Do(ctx context.Context) (decimal.Decimal, error) {
	if s.trace {
		if parent := opentracing.SpanFromContext(ctx); parent != nil {
			if tracer := opentracing.GlobalTracer(); tracer != nil {
				span := tracer.StartSpan("SQL:Company.Sum", opentracing.ChildOf(parent.Context()))
				defer span.Finish()
			}
		}
	}
	...
	if err := sr.Scan(&data); err != nil {
		if repositories.IsRealErr(err) {
			s.core.logger.Error("【Company.Sum】失败", zap.Error(err), zap.ByteString("debug.Stack", debug.Stack()))
		}
