...
// MultiSum 多表SUM数据
func (o *OrderItem) MultiSum(genField field.Expr, sharding []string) *_multiSum {
	return &_multiSum{
		core:          o,
		unscoped:      o.unscoped,
		genField:      genField,
...
// Do 执行多表SUM数据
func (s *_multiSum) Do(ctx context.Context) (decimal.Decimal, map[string]decimal.Decimal, error) {
	if len(s.sharding) == 0 {
	...
	for _, sharding := range s.sharding {
		...
		go func(sharding string) {
			defer func() {
				if r := recover(); r != nil {
					s.core.logger.Error(fmt.Sprintf("【OrderItem.MultiSum.%s】执行异常", sharding), zap.Any("recover", r), zap.ByteString("debug.Stack", debug.Stack()))
			...
			if err := sr.Where(_conditions...).Scan(&data); err != nil {
				if repositories.IsRealErr(err) {
					s.core.logger.Error(fmt.Sprintf("【OrderItem.MultiSum.%s】失败", sharding), zap.Error(err))
				}
				errChan <- err
				return
			}
