...
func (o *OrderItem) MultiTake(sharding []string) *_multiTake {
	return &_multiTake{
		core:          o,
		unscoped:      o.unscoped,
...
// Do 执行获取多表随机一条记录
func (t *_multiTake) Do(ctx context.Context) (*models.OrderItem, error) {
	if len(t.sharding) == 0 {
	...
	for _, sharding := range t.sharding {
		...
		go func(sharding string) {
			defer func() {
				if r := recover(); r != nil {
					t.core.logger.Error(fmt.Sprintf("【OrderItem.MultiTake.%s】执行异常", sharding), zap.Any("recover", r), zap.ByteString("debug.Stack", debug.Stack()))
			...
			if err != nil {
				if repositories.IsRealErr(err) {
					t.core.logger.Error(fmt.Sprintf("【OrderItem.MultiTake.%s】失败", sharding), zap.Error(err))
				}
