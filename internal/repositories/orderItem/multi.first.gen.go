...
func (o *OrderItem) MultiFirst(sharding []string) *_multiFirst {
	return &_multiFirst{
		core:          o,
		unscoped:      o.unscoped,
...
func (f *_multiFirst) Do(ctx context.Context) (*models.OrderItem, error) {
	if len(f.sharding) == 0 {
	...
	for _, sharding := range f.sharding {
		...
		go func(sharding string) {
			defer func() {
				if r := recover(); r != nil {
					f.core.logger.Error(fmt.Sprintf("【OrderItem.MultiFirst.%s】执行异常", sharding), zap.Any("recover", r), zap.ByteString("debug.Stack", debug.Stack()))
			...
			if err != nil {
				if repositories.IsRealErr(err) {
					f.core.logger.Error(fmt.Sprintf("【OrderItem.MultiFirst.%s】失败", sharding), zap.Error(err))
				}
