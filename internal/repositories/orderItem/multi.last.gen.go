...
func (o *OrderItem) MultiLast(sharding []string) *_multiLast {
	return &_multiLast{
		core:          o,
		unscoped:      o.unscoped,
...
func (l *_multiLast) Do(ctx context.Context) (*models.OrderItem, error) {
	if len(l.sharding) == 0 {
	...
	for _, sharding := range l.sharding {
		...
		go func(sharding string) {
			defer func() {
				if r := recover(); r != nil {
					l.core.logger.Error(fmt.Sprintf("【OrderItem.MultiLast.%s】执行异常", sharding), zap.Any("recover", r), zap.ByteString("debug.Stack", debug.Stack()))
			...
			if err != nil {
				if repositories.IsRealErr(err) {
					l.core.logger.Error(fmt.Sprintf("【OrderItem.MultiLast.%s】失败", sharding), zap.Error(err))
				}
