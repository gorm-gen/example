...
// MultiList 获取多表数据列表
func (o *OrderItem) MultiList(sharding []string) *_multiList {
	return &_multiList{
		core:          o,
		unscoped:      o.unscoped,
...
// Do 执行获取多表数据列表
func (l *_multiList) Do(ctx context.Context) ([]*models.OrderItem, int64, error) {
	...
	for k, v := range slList {
		...
		go func(k int, v list.Sl) {
			defer func() {
				if r := recover(); r != nil {
					l.core.logger.Error(fmt.Sprintf("【OrderItem.MultiList.%s】执行异常-1", v.ShardingValue), zap.Any("recover", r), zap.ByteString("debug.Stack", debug.Stack()))
			...
			for kk, vv := range v.List {
				...
				go func(kk int, vv list.Sld) {
					defer func() {
						if r := recover(); r != nil {
							l.core.logger.Error(fmt.Sprintf("【OrderItem.MultiList.%s】执行异常-2", v.ShardingValue), zap.Any("recover", r), zap.ByteString("debug.Stack", debug.Stack()))
					...
					if res, err = lr.Scopes(page.Paginate(vv.Page, vv.PageSize)).Find(); err != nil {
						if repositories.IsRealErr(err) {
							l.core.logger.Error(fmt.Sprintf("【OrderItem.MultiList.%s】失败", v.ShardingValue), zap.Error(err))
						}
						_errChan <- err
						return
					}
