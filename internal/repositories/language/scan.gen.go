...
func (l *Language) Scan(dest interface{}) *_scan {
	return &_scan{
		core:          l,
		unscoped:      l.unscoped,
		dest:          dest,
...
func (s *_scan) Do(ctx context.Context) error {
	if s.trace {
		if parent := opentracing.SpanFromContext(ctx); parent != nil {
			if tracer := opentracing.GlobalTracer(); tracer != nil {
				span := tracer.StartSpan("SQL:Language.Scan", opentracing.ChildOf(parent.Context()))
				defer span.Finish()
			}
		}
	}
	sq := s.core.q.Language
	if s.tx != nil {
		sq = s.tx.Language
	}
	if s.qTx != nil {
	...
	if err := sr.Scan(s.dest); err != nil {
		if repositories.IsRealErr(err) {
			s.core.logger.Error("【Language.Scan】失败", zap.Error(err), zap.ByteString("debug.Stack", debug.Stack()))
		}
		return err
	}
	return nil
