...
// Do 执行获取数据列表
func (l *_list) Do(ctx context.Context) ([]*models.Language, error) {
	if l.trace {
		if parent := opentracing.SpanFromContext(ctx); parent != nil {
			if tracer := opentracing.GlobalTracer(); tracer != nil {
				span := tracer.StartSpan("SQL:Language.List", opentracing.ChildOf(parent.Context()))
				defer span.Finish()
			}
		}
	}
	...
	if err != nil {
		if repositories.IsRealErr(err) {
			l.core.logger.Error("【Language.List】失败", zap.Error(err), zap.ByteString("debug.Stack", debug.Stack()))
		}
		return nil, err
	}
	return list, nil
