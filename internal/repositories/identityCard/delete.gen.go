...
func (d *_delete) Do(ctx context.Context) (int64, error) {
	if d.trace {
		if parent := opentracing.SpanFromContext(ctx); parent != nil {
			if tracer := opentracing.GlobalTracer(); tracer != nil {
				span := tracer.StartSpan("SQL:IdentityCard.Delete", opentracing.ChildOf(parent.Context()))
				defer span.Finish()
			}
		}
	}
	dq := d.core.q.IdentityCard
	if d.tx != nil {
		dq = d.tx.IdentityCard
	}
	if d.qTx != nil {
		dq = d.qTx.IdentityCard
	}
	...
	if err != nil {
		if repositories.IsRealErr(err) {
			d.core.logger.Error("【IdentityCard.Delete】失败", zap.Error(err), zap.ByteString("debug.Stack", debug.Stack()))
		}
		return 0, err
	}
