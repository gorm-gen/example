...
// Sum SUM数据
func (i *IdentityCard) Sum(genField field.Expr) *_sum {
	return &_sum{
		core:          i,
		unscoped:      i.unscoped,
		genField:      genField,
...
func (s *_sum) Do(ctx context.Context) (decimal.Decimal, error) {
	if s.trace {
		if parent := opentracing.SpanFromContext(ctx); parent != nil {
			if tracer := opentracing.GlobalTracer(); tracer != nil {
				span := tracer.StartSpan("SQL:IdentityCard.Sum", opentracing.ChildOf(parent.Context()))
				defer span.Finish()
			}
		}
	}
	sq := s.core.q.IdentityCard
	if s.tx != nil {
		sq = s.tx.IdentityCard
	}
	if s.qTx != nil {
		sq = s.qTx.IdentityCard
	}
	...
	if err := sr.Scan(&data); err != nil {
		if repositories.IsRealErr(err) {
			s.core.logger.Error("【IdentityCard.Sum】失败", zap.Error(err), zap.ByteString("debug.Stack", debug.Stack()))
		}
