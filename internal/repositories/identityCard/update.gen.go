...
// Update 更新数据
func (i *IdentityCard) Update() *_update {
	return &_update{
		core:          i,
		unscoped:      i.unscoped,
...
func (u *_update) Do(ctx context.Context) (int64, error) {
	if u.trace {
		if parent := opentracing.SpanFromContext(ctx); parent != nil {
			if tracer := opentracing.GlobalTracer(); tracer != nil {
				span := tracer.StartSpan("SQL:IdentityCard.Update", opentracing.ChildOf(parent.Context()))
				defer span.Finish()
			}
		}
	}
	...
	uq := u.core.q.IdentityCard
	if u.tx != nil {
		uq = u.tx.IdentityCard
	}
	if u.qTx != nil {
		uq = u.qTx.IdentityCard
	}
	...
	if err != nil {
		if repositories.IsRealErr(err) {
			u.core.logger.Error("【IdentityCard.Update】失败", zap.Error(err), zap.ByteString("debug.Stack", debug.Stack()))
		}
		return 0, err
	}
