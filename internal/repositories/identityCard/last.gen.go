...
// Last 获取最后一条记录（主键降序）
func (i *IdentityCard) Last() *_last {
	return &_last{
		core:          i,
		unscoped:      i.unscoped,
...
func (l *_last) Do(ctx context.Context) (*models.IdentityCard, error) {
	if l.trace {
		if parent := opentracing.SpanFromContext(ctx); parent != nil {
			if tracer := opentracing.GlobalTracer(); tracer != nil {
				span := tracer.StartSpan("SQL:IdentityCard.Last", opentracing.ChildOf(parent.Context()))
				defer span.Finish()
			}
		}
	}
	...
	if err != nil {
		if repositories.IsRealErr(err) {
			l.core.logger.Error("【IdentityCard.Last】失败", zap.Error(err), zap.ByteString("debug.Stack", debug.Stack()))
		}
		return nil, err
	}
	return res, nil
