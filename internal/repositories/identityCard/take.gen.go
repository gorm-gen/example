...
// Take 获取一条记录
func (i *IdentityCard) Take() *_take {
	return &_take{
		core:          i,
		unscoped:      i.unscoped,
...
// Do 执行获取一条记录
func (t *_take) Do(ctx context.Context) (*models.IdentityCard, error) {
	if t.trace {
		if parent := opentracing.SpanFromContext(ctx); parent != nil {
			if tracer := opentracing.GlobalTracer(); tracer != nil {
				span := tracer.StartSpan("SQL:IdentityCard.Take", opentracing.ChildOf(parent.Context()))
				defer span.Finish()
			}
		}
	}
	tq := t.core.q.IdentityCard
	if t.tx != nil {
		tq = t.tx.IdentityCard
	}
	if t.qTx != nil {
		tq = t.qTx.IdentityCard
	}
