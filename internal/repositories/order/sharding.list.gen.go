...
// Do 执行获取分表数据列表
func (l *_shardingList) Do(ctx context.Context) ([]*models.Order, int64, error) {
	if l.trace {
		if parent := opentracing.SpanFromContext(ctx); parent != nil {
			if tracer := opentracing.GlobalTracer(); tracer != nil {
				span := tracer.StartSpan("SQL:Order.ShardingList", opentracing.ChildOf(parent.Context()))
				defer span.Finish()
			}
		}
	}
	...
	for k, v := range slList {
		...
		go func(k int, v list.Sl) {
			defer func() {
				if r := recover(); r != nil {
					l.core.logger.Error(fmt.Sprintf("【Order.ShardingList.%s】执行异常-1", v.ShardingValue), zap.Any("recover", r), zap.ByteString("debug.Stack", debug.Stack()))
			...
			for kk, vv := range v.List {
				...
				go func(kk int, vv list.Sld) {
					defer func() {
						if r := recover(); r != nil {
							l.core.logger.Error(fmt.Sprintf("【Order.ShardingList.%s】执行异常-2", v.ShardingValue), zap.Any("recover", r), zap.ByteString("debug.Stack", debug.Stack()))
