...
// MultiList 获取多表数据列表
func (o *Order) MultiList(sharding []string) *_multiList {
	return &_multiList{
		core:          o,
		unscoped:      o.unscoped,
...
func (l *_multiList) Worker(worker chan struct{}) *_multiList {
	if worker == nil {
		return l
	}
	l.worker = worker
...
// Desc sharding列表倒序
func (l *_multiList) Desc() *_multiList {
	l.asc = false
	return l
}

// Asc sharding列表正序
func (l *_multiList) Asc() *_multiList {
	l.asc = true
	return l
}

// Tx 设置为事务
func (l *_multiList) Tx(tx *query.Query) *_multiList {
	l.tx = tx
	if tx != nil {
		l.qTx = nil
	}
...
// QueryTx 设置为手动事务
func (l *_multiList) QueryTx(tx *query.QueryTx) *_multiList {
	l.qTx = tx
	if tx != nil {
		l.tx = nil
	}
...
// Do 执行获取多表数据列表
func (l *_multiList) Do(ctx context.Context) ([]*models.Order, int64, error) {
	empty := make([]*models.Order, 0)
	if len(l.sharding) == 0 {
		return empty, 0, nil
	}
	// 获取多表数据总数
	multiCount := l.core.MultiCount(l.sharding).
		Worker(l.worker).
		Tx(l.tx).
		QueryTx(l.qTx).
		Where(l.conditionOpts...)
	if l.unscoped {
		multiCount = multiCount.Unscoped()
	}
	count, m, err := multiCount.Do(ctx)
	if err != nil {
		return nil, 0, err
	}
	if count == 0 {
