package order

import (
	"context"
	"fmt"
	"runtime"
	"runtime/debug"
	"sync"

	page "github.com/gorm-gen/paginate/gen"
	"github.com/gorm-gen/sharding/query/list"
	"go.uber.org/zap"
	"gorm.io/gen"
	"gorm.io/gen/field"
	"gorm.io/gorm/clause"

	"example/internal/query"

	"example/internal/repositories"

	"example/internal/models"
)

type multiList struct {
	core          *Order
	tx            *query.Query
	qTx           *query.QueryTx
	page          int
	pageSize      int
	lock          clause.Expression
	unscoped      bool
	selects       []field.Expr
	orderOpts     []OrderOption
	conditionOpts []ConditionOption
	sharding      []string
	worker        chan struct{}
	asc           bool
}

// MultiList 获取多表数据列表
func (o *Order) MultiList(sharding []string) *multiList {
	return &multiList{
		core:          o,
		unscoped:      o.unscoped,
		selects:       make([]field.Expr, 0),
		orderOpts:     make([]OrderOption, 0),
		conditionOpts: make([]ConditionOption, 0),
		sharding:      sharding,
		worker:        make(chan struct{}, runtime.NumCPU()),
	}
}

func (l *multiList) Worker(worker chan struct{}) *multiList {
	if worker == nil {
		return l
	}
	l.worker = worker
	return l
}

// Desc sharding列表倒序
func (l *multiList) Desc() *multiList {
	l.asc = false
	return l
}

// Asc sharding列表正序
func (l *multiList) Asc() *multiList {
	l.asc = true
	return l
}

// Tx 设置为事务
func (l *multiList) Tx(tx *query.Query) *multiList {
	l.tx = tx
	if tx != nil {
		l.qTx = nil
	}
	return l
}

// QueryTx 设置为手动事务
func (l *multiList) QueryTx(tx *query.QueryTx) *multiList {
	l.qTx = tx
	if tx != nil {
		l.tx = nil
	}
	return l
}

func (l *multiList) Select(field ...field.Expr) *multiList {
	l.selects = append(l.selects, field...)
	return l
}

func (l *multiList) ForUpdate() *multiList {
	l.lock = clause.Locking{Strength: clause.LockingStrengthUpdate}
	return l
}

func (l *multiList) ForUpdateSkipLocked() *multiList {
	l.lock = clause.Locking{Strength: clause.LockingStrengthUpdate, Options: clause.LockingOptionsSkipLocked}
	return l
}

func (l *multiList) ForUpdateNoWait() *multiList {
	l.lock = clause.Locking{Strength: clause.LockingStrengthUpdate, Options: clause.LockingOptionsNoWait}
	return l
}

func (l *multiList) ForShare() *multiList {
	l.lock = clause.Locking{Strength: clause.LockingStrengthShare}
	return l
}

func (l *multiList) ForShareSkipLocked() *multiList {
	l.lock = clause.Locking{Strength: clause.LockingStrengthShare, Options: clause.LockingOptionsSkipLocked}
	return l
}

func (l *multiList) ForShareNoWait() *multiList {
	l.lock = clause.Locking{Strength: clause.LockingStrengthShare, Options: clause.LockingOptionsNoWait}
	return l
}

func (l *multiList) Unscoped() *multiList {
	l.unscoped = true
	return l
}

func (l *multiList) Order(opts ...OrderOption) *multiList {
	l.orderOpts = append(l.orderOpts, opts...)
	return l
}

func (l *multiList) Where(opts ...ConditionOption) *multiList {
	l.conditionOpts = append(l.conditionOpts, opts...)
	return l
}

// Page 列表分页
func (l *multiList) Page(page, pageSize uint) *multiList {
	l.page = int(page)
	l.pageSize = int(pageSize)
	return l
}

// Do 执行获取多表数据列表
func (l *multiList) Do(ctx context.Context) ([]*models.Order, int64, error) {
	empty := make([]*models.Order, 0)
	if len(l.sharding) == 0 {
		return empty, 0, nil
	}
	// 获取多表数据总数
	_multiCount := l.core.MultiCount(l.sharding).
		Worker(l.worker).
		Tx(l.tx).
		QueryTx(l.qTx).
		Where(l.conditionOpts...)
	if l.unscoped {
		_multiCount = _multiCount.Unscoped()
	}
	count, m, err := _multiCount.Do(ctx)
	if err != nil {
		return nil, 0, err
	}
	if count == 0 {
		return empty, 0, nil
	}
	// 获取多表分页数据
	svs := make([]*list.St, 0, len(l.sharding))
	for k, v := range m {
		svs = append(svs, &list.St{
			ShardingValue: k,
			Total:         uint64(v),
		})
	}
	listOpts := []list.Option{
		list.WithPage(uint64(l.page)),
		list.WithPageSize(uint64(l.pageSize)),
	}
	if l.asc {
		listOpts = append(listOpts, list.WithAsc())
	} else {
		listOpts = append(listOpts, list.WithDesc())
	}
	slList := list.New(svs, listOpts...).Analysis()
	slList.ToSliceIndex()
	// 获取多表数据
	lq := l.core.q.Order
	if l.tx != nil {
		lq = l.tx.Order
	}
	if l.qTx != nil {
		lq = l.qTx.Order
	}
	var conditions []gen.Condition
	if len(l.conditionOpts) > 0 {
		conditions = make([]gen.Condition, 0, len(l.conditionOpts))
		for _, opt := range l.conditionOpts {
			conditions = append(conditions, opt(l.core))
		}
	}
	var fieldExpr []field.Expr
	if len(l.selects) > 0 {
		fieldExpr = make([]field.Expr, 0, len(l.selects))
		if l.core.newTableName == nil {
			fieldExpr = append(fieldExpr, l.selects...)
		} else {
			for _, v := range l.selects {
				fieldExpr = append(fieldExpr, field.NewField(*l.core.newTableName, v.ColumnName().String()))
			}
		}
	}
	var orders []field.Expr
	if len(l.orderOpts) > 0 {
		orders = make([]field.Expr, 0, len(l.orderOpts))
		for _, opt := range l.orderOpts {
			orders = append(orders, opt(l.core))
		}
	}
	wg := sync.WaitGroup{}
	_count_ := int64(0)
	_list_ := make([][]*models.Order, len(slList))
	endChan := make(chan struct{})
	errChan := make(chan error)
	for k, v := range slList {
		l.worker <- struct{}{}
		wg.Add(1)
		_count_ += v.Num
		go func(k int, v list.Sl) {
			defer func() {
				if r := recover(); r != nil {
					l.core.logger.Error(fmt.Sprintf("【Order.MultiList.%s】执行异常-1", v.ShardingValue), zap.Any("recover", r), zap.ByteString("debug.Stack", debug.Stack()))
					errChan <- fmt.Errorf("recovered from panic: %v", r)
				}
			}()
			defer func() {
				<-l.worker
			}()
			defer wg.Done()
			_wg := &sync.WaitGroup{}
			_endChan := make(chan struct{})
			_errChan := make(chan error)
			__list := make([][]*models.Order, len(v.List))
			for kk, vv := range v.List {
				l.worker <- struct{}{}
				_wg.Add(1)
				go func(kk int, vv list.Sld) {
					defer func() {
						if r := recover(); r != nil {
							l.core.logger.Error(fmt.Sprintf("【Order.MultiList.%s】执行异常-2", v.ShardingValue), zap.Any("recover", r), zap.ByteString("debug.Stack", debug.Stack()))
							_errChan <- fmt.Errorf("recovered from panic: %v", r)
						}
					}()
					defer func() {
						<-l.worker
					}()
					defer _wg.Done()
					_conditions := make([]gen.Condition, len(conditions))
					copy(_conditions, conditions)
					_conditions = append(_conditions, ConditionSharding(v.ShardingValue)(l.core))
					lr := lq.WithContext(ctx)
					if len(fieldExpr) > 0 {
						lr = lr.Select(fieldExpr...)
					}
					if l.unscoped {
						lr = lr.Unscoped()
					}
					if (l.tx != nil || l.qTx != nil) && l.lock != nil {
						lr = lr.Clauses(l.lock)
					}
					lr = lr.Where(_conditions...)
					if len(orders) > 0 {
						lr = lr.Order(orders...)
					}
					var res []*models.Order
					if res, err = lr.Scopes(page.Paginate(vv.Page, vv.PageSize)).Find(); err != nil {
						if repositories.IsRealErr(err) {
							l.core.logger.Error(fmt.Sprintf("【Order.MultiList.%s】失败", v.ShardingValue), zap.Error(err))
						}
						_errChan <- err
						return
					}
					__list[kk] = res[vv.Start:vv.End]
				}(kk, vv)
			}
			go func() {
				_wg.Wait()
				_endChan <- struct{}{}
			}()
			select {
			case <-_endChan:
				___list := make([]*models.Order, 0, v.Num)
				for _, vvv := range __list {
					___list = append(___list, vvv...)
				}
				_list_[k] = ___list
				return
			case err = <-_errChan:
				errChan <- err
				return
			}
		}(k, v)
	}
	go func() {
		wg.Wait()
		endChan <- struct{}{}
	}()
	select {
	case <-endChan:
		__list := make([]*models.Order, 0, _count_)
		for _, v := range _list_ {
			__list = append(__list, v...)
		}
		return __list, count, nil
	case err = <-errChan:
		return nil, 0, err
	}
}
