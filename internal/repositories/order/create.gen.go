...
func (c *_create) Values(values ...*models.Order) *_create {
...
func (c *_create) Do(ctx context.Context) (err error) {
	if c.trace {
		if parent := opentracing.SpanFromContext(ctx); parent != nil {
			if tracer := opentracing.GlobalTracer(); tracer != nil {
				span := tracer.StartSpan("SQL:Order.Create", opentracing.ChildOf(parent.Context()))
				defer span.Finish()
			}
		}
	}
	...
	if err != nil {
		if repositories.IsRealErr(err) {
			c.core.logger.Error("【Order.Create】失败", zap.Error(err), zap.ByteString("debug.Stack", debug.Stack()))
		}
		return err
	}
	return nil
