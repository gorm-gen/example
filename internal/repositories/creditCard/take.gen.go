...
// Do 执行获取一条记录
func (t *_take) Do(ctx context.Context) (*models.CreditCard, error) {
	if t.trace {
		if parent := opentracing.SpanFromContext(ctx); parent != nil {
			if tracer := opentracing.GlobalTracer(); tracer != nil {
				span := tracer.StartSpan("SQL:CreditCard.Take", opentracing.ChildOf(parent.Context()))
				defer span.Finish()
			}
		}
	}
	tq := t.core.q.CreditCard
	if t.tx != nil {
		tq = t.tx.CreditCard
	}
	if t.qTx != nil {
	...
	if err != nil {
		if repositories.IsRealErr(err) {
			t.core.logger.Error("【CreditCard.Take】失败", zap.Error(err), zap.ByteString("debug.Stack", debug.Stack()))
		}
		return nil, err
	}
	return res, nil
