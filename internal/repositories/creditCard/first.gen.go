...
// First 获取第一条记录（主键升序）
func (c *CreditCard) First() *_first {
	return &_first{
		core:          c,
		unscoped:      c.unscoped,
...
// Do 执行获取第一条记录（主键升序）
func (f *_first) Do(ctx context.Context) (*models.CreditCard, error) {
	if f.trace {
		if parent := opentracing.SpanFromContext(ctx); parent != nil {
			if tracer := opentracing.GlobalTracer(); tracer != nil {
				span := tracer.StartSpan("SQL:CreditCard.First", opentracing.ChildOf(parent.Context()))
				defer span.Finish()
			}
		}
	}
	fq := f.core.q.CreditCard
	if f.tx != nil {
		fq = f.tx.CreditCard
	}
	if f.qTx != nil {
	...
	if err != nil {
		if repositories.IsRealErr(err) {
			f.core.logger.Error("【CreditCard.First】失败", zap.Error(err), zap.ByteString("debug.Stack", debug.Stack()))
		}
		return nil, err
	}
	return res, nil
