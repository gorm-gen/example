...
// Delete 删除数据
func (c *CreditCard) Delete() *_delete {
	return &_delete{
		core:          c,
		unscoped:      c.unscoped,
...
func (d *_delete) Do(ctx context.Context) (int64, error) {
	if d.trace {
		if parent := opentracing.SpanFromContext(ctx); parent != nil {
			if tracer := opentracing.GlobalTracer(); tracer != nil {
				span := tracer.StartSpan("SQL:CreditCard.Delete", opentracing.ChildOf(parent.Context()))
				defer span.Finish()
			}
		}
	}
	dq := d.core.q.CreditCard
	if d.tx != nil {
		dq = d.tx.CreditCard
	}
	if d.qTx != nil {
	...
	if err != nil {
		if repositories.IsRealErr(err) {
			d.core.logger.Error("【CreditCard.Delete】失败", zap.Error(err), zap.ByteString("debug.Stack", debug.Stack()))
		}
		return 0, err
	}
