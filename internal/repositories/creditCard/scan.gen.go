...
func (c *CreditCard) Scan(dest interface{}) *_scan {
	return &_scan{
		core:          c,
		unscoped:      c.unscoped,
		dest:          dest,
...
func (s *_scan) Do(ctx context.Context) error {
	if s.trace {
		if parent := opentracing.SpanFromContext(ctx); parent != nil {
			if tracer := opentracing.GlobalTracer(); tracer != nil {
				span := tracer.StartSpan("SQL:CreditCard.Scan", opentracing.ChildOf(parent.Context()))
				defer span.Finish()
			}
		}
	}
	...
	if err := sr.Scan(s.dest); err != nil {
		if repositories.IsRealErr(err) {
			s.core.logger.Error("【CreditCard.Scan】失败", zap.Error(err), zap.ByteString("debug.Stack", debug.Stack()))
		}
		return err
	}
	return nil
