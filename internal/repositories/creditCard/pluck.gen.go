...
func (c *CreditCard) Pluck(genField field.Expr, dest interface{}) *_pluck {
	return &_pluck{
		core:          c,
		unscoped:      c.unscoped,
		genField:      genField,
...
func (p *_pluck) Do(ctx context.Context) error {
	if p.trace {
		if parent := opentracing.SpanFromContext(ctx); parent != nil {
			if tracer := opentracing.GlobalTracer(); tracer != nil {
				span := tracer.StartSpan("SQL:CreditCard.Pluck", opentracing.ChildOf(parent.Context()))
				defer span.Finish()
			}
		}
	}
	pq := p.core.q.CreditCard
	if p.tx != nil {
		pq = p.tx.CreditCard
	}
	if p.qTx != nil {
	...
	if err := pr.Pluck(column, p.dest); err != nil {
		if repositories.IsRealErr(err) {
			p.core.logger.Error("【CreditCard.Pluck】失败", zap.Error(err), zap.ByteString("debug.Stack", debug.Stack()))
		}
		return err
	}
	return nil
