...
// Do 执行获取最后一条记录（主键降序）
func (l *_last) Do(ctx context.Context) (*models.CreditCard, error) {
	if l.trace {
		if parent := opentracing.SpanFromContext(ctx); parent != nil {
			if tracer := opentracing.GlobalTracer(); tracer != nil {
				span := tracer.StartSpan("SQL:CreditCard.Last", opentracing.ChildOf(parent.Context()))
				defer span.Finish()
			}
		}
	}
	lq := l.core.q.CreditCard
	if l.tx != nil {
		lq = l.tx.CreditCard
	}
	if l.qTx != nil {
	...
	if err != nil {
		if repositories.IsRealErr(err) {
			l.core.logger.Error("【CreditCard.Last】失败", zap.Error(err), zap.ByteString("debug.Stack", debug.Stack()))
		}
		return nil, err
	}
	return res, nil
