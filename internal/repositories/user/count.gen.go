...
func (c *_count) Do(ctx context.Context) (int64, error) {
	if c.trace {
		if parent := opentracing.SpanFromContext(ctx); parent != nil {
			if tracer := opentracing.GlobalTracer(); tracer != nil {
				span := tracer.StartSpan("SQL:User.Count", opentracing.ChildOf(parent.Context()))
				defer span.Finish()
			}
		}
	}
	...
	if err != nil {
		if repositories.IsRealErr(err) {
			c.core.logger.Error("【User.Count】失败", zap.Error(err), zap.ByteString("debug.Stack", debug.Stack()))
		}
		return 0, err
	}
	return count, nil
