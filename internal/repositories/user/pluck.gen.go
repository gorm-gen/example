...
func (u *User) Pluck(genField field.Expr, dest interface{}) *_pluck {
	return &_pluck{
		core:          u,
		unscoped:      u.unscoped,
		genField:      genField,
...
func (p *_pluck) Do(ctx context.Context) error {
	if p.trace {
		if parent := opentracing.SpanFromContext(ctx); parent != nil {
			if tracer := opentracing.GlobalTracer(); tracer != nil {
				span := tracer.StartSpan("SQL:User.Pluck", opentracing.ChildOf(parent.Context()))
				defer span.Finish()
			}
		}
	}
	...
	if err := pr.Pluck(column, p.dest); err != nil {
		if repositories.IsRealErr(err) {
			p.core.logger.Error("【User.Pluck】失败", zap.Error(err), zap.ByteString("debug.Stack", debug.Stack()))
		}
		return err
	}
	return nil
