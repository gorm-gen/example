...
// Delete 删除数据
func (u *UserLanguage) Delete() *_delete {
	return &_delete{
		core:          u,
		unscoped:      u.unscoped,
...
func (d *_delete) Do(ctx context.Context) (int64, error) {
	if d.trace {
		if parent := opentracing.SpanFromContext(ctx); parent != nil {
			if tracer := opentracing.GlobalTracer(); tracer != nil {
				span := tracer.StartSpan("SQL:UserLanguage.Delete", opentracing.ChildOf(parent.Context()))
				defer span.Finish()
			}
		}
	}
	dq := d.core.q.UserLanguage
	if d.tx != nil {
		dq = d.tx.UserLanguage
	}
	if d.qTx != nil {
		dq = d.qTx.UserLanguage
	}
	...
	if err != nil {
		if repositories.IsRealErr(err) {
			d.core.logger.Error("【UserLanguage.Delete】失败", zap.Error(err), zap.ByteString("debug.Stack", debug.Stack()))
		}
		return 0, err
	}
