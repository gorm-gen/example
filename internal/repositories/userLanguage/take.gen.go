...
// Take 获取一条记录
func (u *UserLanguage) Take() *_take {
	return &_take{
		core:          u,
		unscoped:      u.unscoped,
...
// Do 执行获取一条记录
func (t *_take) Do(ctx context.Context) (*models.UserLanguage, error) {
	if t.trace {
		if parent := opentracing.SpanFromContext(ctx); parent != nil {
			if tracer := opentracing.GlobalTracer(); tracer != nil {
				span := tracer.StartSpan("SQL:UserLanguage.Take", opentracing.ChildOf(parent.Context()))
				defer span.Finish()
			}
		}
	}
	tq := t.core.q.UserLanguage
	if t.tx != nil {
		tq = t.tx.UserLanguage
	}
	if t.qTx != nil {
		tq = t.qTx.UserLanguage
	}
	...
	if err != nil {
		if repositories.IsRealErr(err) {
			t.core.logger.Error("【UserLanguage.Take】失败", zap.Error(err), zap.ByteString("debug.Stack", debug.Stack()))
		}
		return nil, err
	}
	return res, nil
