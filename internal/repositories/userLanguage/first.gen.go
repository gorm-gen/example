...
// First 获取第一条记录（主键升序）
func (u *UserLanguage) First() *_first {
	return &_first{
		core:          u,
		unscoped:      u.unscoped,
...
// Do 执行获取第一条记录（主键升序）
func (f *_first) Do(ctx context.Context) (*models.UserLanguage, error) {
	if f.trace {
		if parent := opentracing.SpanFromContext(ctx); parent != nil {
			if tracer := opentracing.GlobalTracer(); tracer != nil {
				span := tracer.StartSpan("SQL:UserLanguage.First", opentracing.ChildOf(parent.Context()))
				defer span.Finish()
			}
		}
	}
	fq := f.core.q.UserLanguage
	if f.tx != nil {
		fq = f.tx.UserLanguage
	}
	if f.qTx != nil {
		fq = f.qTx.UserLanguage
	}
	...
	if err != nil {
		if repositories.IsRealErr(err) {
			f.core.logger.Error("【UserLanguage.First】失败", zap.Error(err), zap.ByteString("debug.Stack", debug.Stack()))
		}
		return nil, err
	}
	return res, nil
