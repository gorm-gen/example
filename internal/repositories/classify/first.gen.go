...
// Do 执行获取第一条记录（主键升序）
func (f *_first) Do(ctx context.Context) (*models.Classify, error) {
	if f.trace {
		if parent := opentracing.SpanFromContext(ctx); parent != nil {
			if tracer := opentracing.GlobalTracer(); tracer != nil {
				span := tracer.StartSpan("SQL:Classify.First", opentracing.ChildOf(parent.Context()))
				defer span.Finish()
			}
		}
	}
	fq := f.core.q.Classify
	if f.tx != nil {
		fq = f.tx.Classify
	}
	if f.qTx != nil {
	...
	if err != nil {
		if repositories.IsRealErr(err) {
			f.core.logger.Error("【Classify.First】失败", zap.Error(err), zap.ByteString("debug.Stack", debug.Stack()))
		}
		return nil, err
	}
	return res, nil
