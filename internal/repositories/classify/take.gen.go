...
// Do 执行获取一条记录
func (t *_take) Do(ctx context.Context) (*models.Classify, error) {
	if t.trace {
		if parent := opentracing.SpanFromContext(ctx); parent != nil {
			if tracer := opentracing.GlobalTracer(); tracer != nil {
				span := tracer.StartSpan("SQL:Classify.Take", opentracing.ChildOf(parent.Context()))
				defer span.Finish()
			}
		}
	}
	tq := t.core.q.Classify
	if t.tx != nil {
		tq = t.tx.Classify
	}
	if t.qTx != nil {
	...
	if err != nil {
		if repositories.IsRealErr(err) {
			t.core.logger.Error("【Classify.Take】失败", zap.Error(err), zap.ByteString("debug.Stack", debug.Stack()))
		}
		return nil, err
	}
	return res, nil
